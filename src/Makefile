...
ifeq ($(PLATFORM),linux)
  ...
  ifeq ($(USE_LOCAL_HEADERS),1)
	...
	ifeq ($(USE_WEBOS),1)
		CLIENT_CFLAGS += -I$(NANODIR)/inc -DWEBOS
	endif
...
makedirs:
	...
	@if [ ! -d $(B)/nanoGL ];then $(MKDIR) $(B)/nanoGL;fi
...
ifeq ($(USE_WEBOS),1)
  DO_NANO=g++ -Winline -I$(NANODIR)/inc -o $@ -c $< 
else
	DO_NANO=
endif

$(B)/nanoGL/%.o: $(NANODIR)/src/%.cpp
	$(DO_NANO)
