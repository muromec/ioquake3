...
static void GLimp_DetectAvailableModes(void)
{
	...
	#ifdef WEBOS
	modes = SDL_ListModes( videoInfo->vfmt, SDL_OPENGLES | SDL_FULLSCREEN );
	#else
...
static int GLimp_SetMode(int mode, qboolean fullscreen, qboolean noborder)
{
	...
	for (i = 0; i < 16; i++)
	{
		...
		#ifdef WEBOS
		SDL_GL_SetAttribute(SDL_GL_CONTEXT_MAJOR_VERSION, 1); SDL_GL_SetAttribute(SDL_GL_CONTEXT_MINOR_VERSION, 1);
		//if (!(vidscreen = SDL_SetVideoMode(320, 480, colorbits, flags)))
...
static void GLimp_InitExtensions( void )
{
	...
	if ( GLimp_HaveExtension( "GL_ARB_multitexture" ) )
	{
		if ( r_ext_multitexture->value )
		{
			...
			{
				GLint glint = 0;
				// COMPLETELY not sure about this
				#ifdef WEBOS
